    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-6xl mx-auto">
      ...
      >
        ...
        {scanning ? (
          <div className="flex flex-col items-center gap-3">
            ...
            <p className="text-sm text-gray-600">Scanning document for sensitive information...</p>
          </div>
        ) : (
          <>
            ...
            <p className="text-xs text-gray-400 mt-3">PDF, DOCX, or XLSX up to 10 MB</p>
          </>
        )}
      </div>
      ...
      {result && (
        <div className="mt-6 space-y-6">
          {/* Summary Card */}
          <div className={`bg-white border rounded-xl p-6 ${levelBorderColors[result.level] || 'border-gray-200'}`}>
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
              ...
              <div className="flex items-center gap-3">
                <span className={`px-3 py-1 rounded-full text-sm font-medium ${levelColors[result.level] || 'bg-gray-100 text-gray-800'}`}>
            ...
            <p className="mt-3 text-sm text-gray-600">{result.explanation}</p>
          </div>

          {/* Stats Grid */}
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Entities Found</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{result.entitiesFound}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Entities Redacted</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{result.entitiesRedacted}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Entity Score</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{result.breakdown.entityScore}</p>
            </div>
            <div className="bg-white border border-gray-200 rounded-lg p-4">
              <p className="text-xs text-gray-500 uppercase tracking-wide">Legal Boost</p>
              <p className="text-2xl font-bold text-gray-900 mt-1">{result.breakdown.legalBoost}</p>
            </div>
          </div>
          ...
          {Object.keys(entityGroups).length > 0 && (
            <div className="bg-white border border-gray-200 rounded-xl p-6">
              <h3 className="text-sm font-semibold text-gray-900 mb-4">Detected Entity Types</h3>
          ...
          {result.entities.length > 0 && (
            <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
              <div className="p-4 border-b border-gray-200">
                <h3 className="text-sm font-semibold text-gray-900">Detected Entities</h3>
              </div>
                ...
                <table className="w-full text-sm">
                  <thead className="bg-gray-50">
                    <tr>
                      ...
                      <th className="text-left px-4 py-2 text-xs font-medium text-gray-500 uppercase">Matched Text</th>
                    ...
                    {result.entities.slice(0, 50).map((entity, i) => (
                      <tr key={i} className="hover:bg-gray-50">
                        <td className="px-4 py-2">
                          <span className="px-2 py-0.5 bg-iron-50 text-iron-700 rounded text-xs font-medium">
                            {entity.type.replace(/_/g, ' ')}
                          </span>
                        </td>
                        <td className="px-4 py-2 text-gray-700 font-mono text-xs max-w-xs truncate">
          ...
          <div className="bg-white border border-gray-200 rounded-xl overflow-hidden">
            <div className="p-4 border-b border-gray-200 flex items-center justify-between">
              <h3 className="text-sm font-semibold text-gray-900">Redacted Document</h3>
              <button
            ...
            <pre className="p-4 text-xs text-gray-700 font-mono whitespace-pre-wrap max-h-96 overflow-y-auto bg-gray-50">
