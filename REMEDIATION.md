...
| Post-plan · Grok + Mistral adapters (Item 12) | ✅ **shipped** — previously deferred feature-add. xAI Grok (grok.com + x.com) and Mistral Chat (chat.mistral.ai) join the adapter registry. Both use wire-level fetch interception, OpenAI-compatible SSE delta parsing, and the shared `defaultIsValidPromptInput` validator from Design Systems Issue #11. | `extension/src/content/adapters/{grok,mistral}.ts`, `registry.ts`, `index.ts`, `manifest.json` |
